...
const CalendarEntryModal: React.FC<CalendarEntryModalProps> = ({
  ...
  const initialNotes = ''; // Leave notes empty for user to fill
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('🔍 [CALENDAR-MODAL] Processing shopping links...');
      console.log('📦 [CALENDAR-MODAL] Selected products received:', selectedProducts);
      console.log('🔗 [CALENDAR-MODAL] Processed links:', processedLinks);
      ...
      const shoppingLinksWithImages = processedLinks.map((link, index) => {
        ...
        console.log(`🔎 [CALENDAR-MODAL] Link ${index}: ${link.url.substring(0, 50)}...`);
        console.log(`  - Matching product found: ${!!matchingProduct}`);
        console.log(`  - Image URL: ${matchingProduct?.imageUrl?.substring(0, 50) || 'NONE'}`);
      ...
      // Determine outfit image URL - prioritize shopping link images, then outfit image
      const outfitImageUrl = 
        shoppingLinksWithImages.length > 0 && shoppingLinksWithImages[0]?.imageUrl
          ? shoppingLinksWithImages[0].imageUrl
          : outfitItems[0]?.imageUrl || outfit?.image || outfit?.imageUrl || '';

      console.log('🖼️ [CALENDAR-MODAL] Outfit image URL to save:', outfitImageUrl?.substring(0, 80));
