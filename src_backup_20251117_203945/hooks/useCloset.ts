...
export const useCloset = () => {
  ...
  const loadItems = useCallback(async () => {
    try {
      ...
      // Map legacy 'sweaters' category to 'outerwear' for backward compatibility
      const mappedData = (data || []).map(item => ({
        ...item,
        category: item.category === 'sweaters' ? 'outerwear' : item.category
      })) as ClothingItem[];
  ...
  const addItem = useCallback(async (item: ClothingItemInput): Promise<ClothingItem | null> => {
    try {
      ...
      const { data: recentItems, error: checkError } = await supabase
        .from('clothing_items')
      ...
      if (checkError) {
        console.warn('⚠️ [CLOSET-HOOK] Duplicate check failed:', checkError);
        // Continue with insert anyway
      } else if (recentItems && recentItems.length > 0) {
        console.warn('⚠️ [CLOSET-HOOK] Duplicate item detected, skipping insert:', {
          name: item.name,
          category: item.category,
          existingId: recentItems[0].id,
          createdAt: recentItems[0].created_at
        });
        // Return the existing item instead
        const existingItem = items.find(i => i.id === recentItems[0].id);
        if (existingItem) {
          return existingItem;
        }
        // If not in local state yet, fetch it
        const { data: fullItem } = await supabase
          .from('clothing_items')
          .select('*')
          .eq('id', recentItems[0].id)
          .single();
        return fullItem as ClothingItem;
      }
