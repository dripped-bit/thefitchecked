...
class ClosetAnalyticsService {
  ...
  private async calculateAnalytics(userId: string): Promise<AnalyticsData> {
    ...
    // NEW: Calculate unworn items (3+ months threshold)
    const unwornData = this.calculateUnwornItems(safeItems, 3);
    ...
    return {
      ...
      unwornItems: unwornData.count,
      unwornValue: unwornData.value,
      unwornByCategory: unwornData.byCategory
    };
  }

  /**
   * Calculate unworn items (items not worn in X months or never worn)
   */
  private calculateUnwornItems(
    items: any[],
    monthsThreshold: number = 3
  ): { count: number; value: number; byCategory: { category: string; count: number; value: number }[] } {
    const thresholdDate = new Date();
    thresholdDate.setMonth(thresholdDate.getMonth() - monthsThreshold);

    // Filter items that haven't been worn in threshold period
    const unwornItems = items.filter(item => {
      if (!item.last_worn) return true; // Never worn
      const lastWorn = new Date(item.last_worn);
      return lastWorn < thresholdDate; // Not worn in threshold period
    });

    const count = unwornItems.length;
    const value = unwornItems.reduce((sum, item) => sum + (item.price || 0), 0);

    // Group by category
    const byCategory = unwornItems.reduce((acc, item) => {
      const cat = item.category || 'uncategorized';
    ...
    console.log(`📊 [ANALYTICS] Found ${count} unworn items worth $${value.toFixed(2)}`);
    return { count, value, byCategory };
  }

  private async analyzeColors(items: any[]): Promise<ColorData[]> {
    // Extract colors from items (group Unknown/empty with Mixed/Other)
    const colorCounts = items.reduce((acc, item) => {
      let color = (item.color || '').trim();
      // FIX: Group Unknown, empty, or generic colors as "Mixed/Other"
...
- Include all colors from the list`
      ...
      console.warn('⚠️ [ANALYTICS] Claude error, using fallback colors:', error instanceof Error ? error.message : JSON.stringify(error));
      return this.getFallbackColors(colorCounts, totalItems);
    }
  }

  private async calculateBestValue(items: any[], outfits: any[]): Promise<BestValueItem[]> {
    // First try: Use actual times_worn data (items with real wear tracking)
...
Return JSON: {"bestValue": [{"id": "...", "estimatedWears": 50, "valueReason": "Versatile basic"}]}`,
      ...
      if (response && response.bestValue && Array.isArray(response.bestValue)) {
        return response.bestValue.slice(0, 3).map((aiItem: any) => {
          ...
          return {
            ...
            name: item.name,
            price: item.price,
            timesWorn: 0, // Mark as estimated (0 = not tracked)
            costPerWear,
      ...
      console.error('❌ [ANALYTICS] AI estimation failed:', error instanceof Error ? error.message : JSON.stringify(error));
    }
    ...
    return itemsWithPrice
      ...
      .map(item => ({
        ...
        name: item.name,
        price: item.price,
        timesWorn: 0, // Mark as not tracked
        costPerWear: item.price / 10, // Estimated
        stars: 3,
        image_url: item.image_url,
        thumbnail_url: item.thumbnail_url,
        category: item.category
